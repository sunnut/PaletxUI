...
import {PxButtonState} from './button-state';

@Directive({selector: '[pxButton]', providers: [DomHandler]})
export class PxButtonDirective implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    if (this.icon) {
      ...
      iconElement.className =
          iconPosClass + ' ui-c iconfont px-icon-' + this.icon;
      this.el.nativeElement.appendChild(iconElement);
    }

    let iconAnimationElement = document.createElement('span');
    iconAnimationElement.className =
        'ui-button-icon-left ui-c iconfont px-icon-circle-o-notch px-spin';
    ...
    if (this.state) {
      ...
      if (this.state === PxButtonState.DOING) {
        if (spanElement) {
  ...
  setIconELement(isShowAnimation: boolean) {
    ...
    let iconAnimationElement = this.domHandler.findSingle(
        this.el.nativeElement, '.iconfont.px-icon-circle-o-notch.px-spin');
