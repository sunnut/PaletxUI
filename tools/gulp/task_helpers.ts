...
export function vendorTask() {
  return () => gulpMerge(
    NPM_VENDOR_FILES.map(root => {
      const glob = path.join(PROJECT_ROOT, 'node_modules', root, '**/*.+(js|js.map|css)');
...
export function concatCss(dest:string, src: string[], cssFile: string){
  return () => {
    return gulp.src(src)
           .pipe(gulpConcatCss(cssFile))
           .pipe(gulp.dest(dest));
  }
}

export function base64BuildTask(src: string, dest: string) {
  return () => {
    return gulp.src(src)
        .pipe(base64())
        .pipe(gulp.dest(dest));
...
export function uglifyCss(src: string, dest: string) {
  return () => {
    return gulp.src(src)
        .pipe(minifycss())
